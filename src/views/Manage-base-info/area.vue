          ...
          <div class="details-top">
            <div class="storage">
              <el-form-item label="仓库名称" prop="warehouseName">
                <el-input
                  placeholder="请输入"
                  style="width: 300px"
                  v-model="areaList.warehouseName"
                ></el-input>
              </el-form-item>
            </div>
            ...
            <div class="storage">
              <el-form-item label="仓库状态" prop="status">
                <el-select v-model="areaList.status" placeholder="请选择">
                  <el-option
    ...
    <el-card>
      <div class="add-warehouse">
        <el-button
          type="success"
          round
          @click="$router.push('/manage-base-info/warehouse/areaDetails/null')"
          >新增库区</el-button
        >
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="name" label="库区名称"> </el-table-column>
        <el-table-column prop="temperatureType" label="温度类型">
          <template v-slot="{ row }">
            <span v-if="row.temperatureType === 'CW'">常温</span>
            <span v-else-if="row.temperatureType === 'LC'">冷藏</span>
            <span v-else>恒温</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="库区状态">
          <template v-slot="{ row }">
            <span v-if="row.status === 1">启用</span>
            <span>停用</span>
          </template>
        ...
        <el-table-column prop="includedNum" label="库位数量"> </el-table-column>
...
<script>
import { getReservoirList, getEditList } from '../../api/reservoirArea'
export default {
  data() {
    return {
      labelPosition: 'top',
  ...
  methods: {
    ...
    async getReservoirList() {
      const res = await getReservoirList(this.areaList)
      // console.log(res)
      this.tableData = res.data.data.records
      this.total = res.data.data.total - 0
    },
    sizeChange(page) {
      this.areaList.current = page
      this.getReservoirList()
    },
    currentChange(page) {
      this.areaList.size = page
      this.getReservoirList()
    },

    // 搜索
    async search() {
      await getReservoirList(this.areaList)
      this.getReservoirList()
    },
    // 重置
    async reset() {
    ...
    del() {
      this.$notify({
        title: '提示',
        message: ('i', { style: 'color: teal' }, '不可以删除哦~')
      })
    },
    // 停用/(启用
    async isStatus(row) {
